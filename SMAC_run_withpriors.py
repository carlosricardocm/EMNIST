...
statsfilename = smac_outdir+"/EMNIST_HPOFacade_with_priors.csv"
...
config = []
for j in range(n_memories):
    config.append(UniformIntegerHyperparameter(str(j)+"_tolerance", lower=0, upper=maxtol, default_value=prior_tolerance)) #(1, 10), default=4))
    #config.append(UniformFloatHyperparameter(str(j)+"_sigma", lower=0.0, upper=hl, default_value=prior_sigma))
    config.append(UniformFloatHyperparameter(str(j)+"_iota", lower=0.0, upper=hl, default_value=prior_iota))
    config.append(UniformFloatHyperparameter(str(j)+"_kappa", lower=0.0, upper=hl, default_value=prior_kappa))
